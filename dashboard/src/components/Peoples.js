...
class Peoples extends React.Component {
  ...
  handleKey = (e) => {
    ...
    got.houses.map((house) => house.people.forEach((p) => allNames.push(p)))

    this.setState({
      name: allNames.filter((p) => p.name.toUpperCase().includes(e.target.value.toUpperCase()))
    })

  }

  render() {
    let peoples = this.state.name
    return (
      <div>
        <div className='got-input'>
          <input placeholder='search' onKeyUp={this.handleKey} />
        </div>
        <div className='grid grid-container justify-around'>
          {
            peoples ? <R people={peoples} /> : <N houses={got.houses} />
          }
        </div>
      </div>
    )
...
function N(props) {
  return (
    props.houses.map((house) => house.people.map((p, i) => {
      return (
        <div key={i} className='grid-item '>
          <div className='flex align-center'>
            <img src={p.image} alt={p.name} className='got-avatar' />
